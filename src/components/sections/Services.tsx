...
export function Services() {
  ...
  return (
    <section id="services" className="mx-auto max-w-7xl px-4 py-16 sm:px-6 sm:py-24">
      <div ref={ref} className="reveal">
        <p className="text-eyebrow">What we do</p>
        <h2 className="mt-3 max-w-2xl text-balance font-display text-3xl font-extrabold leading-[1.02] sm:text-4xl md:text-5xl">
        ...
        {services.map((service) => {
          ...
          return (
            <li key={service.title} data-reveal-child className="reveal">
              <Link
                to={service.to as "/"}
                className="group relative flex h-full flex-col overflow-hidden rounded-xl border border-border bg-surface/60 p-6 transition-all duration-300 hover:-translate-y-1 hover:border-primary/45 hover:bg-surface-2/70"
              >
                <span
                  aria-hidden="true"
                ...
                <span className="mt-5 inline-flex items-center gap-1.5 font-display text-xs font-bold uppercase tracking-[0.16em] text-primary">
                  Read more
